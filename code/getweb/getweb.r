...
#    close(url)
#    filename<-paste(DipMex, "/data/fromWeb/tablas/57/",tabs57[n],".txt",sep="")
...
#    close(url)
#    filename<-paste(DipMex, "/data/fromWeb/tablas/58/",tabs58[n],".txt",sep="")
#    write.table(tmp, filename, sep="\t")
#}
...
#    close(url)
#    filename<-paste(DipMex, "/data/fromWeb/tablas/60/",tabs60[n],".txt",sep="")
#    write.table(tmp, filename, sep="\t")
#}
...
#    close(url)
#    filename<-paste(DipMex, "/data/fromWeb/tablas/root/",tabsrt[n],".txt",sep="")
#    write.table(tmp, filename, sep="\t")
#}
...
list <- readLines(paste(DipMex, "/data/fromWeb/tablas/59/list59.txt", sep=""))
#
...
#    close(url)
#    filename<-paste(DipMex, "/data/fromWeb/tablas/59/",tabs59[n],".txt",sep="")
#    write.table(tmp, filename, sep="\t")
#}
...
path<-c("http://gaceta.diputados.gob.mx/Gaceta/Votaciones/59/")
dir<-paste(DipMex, "/data/fromWeb/tablas/59/", sep="")
#
...
#setwd(paste(DipMex, "/data/fromWeb/tablas/59", sep="")
...
dir<-paste(DipMex, "/data/fromWeb/tablas/61/", sep="")
#
...
#n <- 5
for (n in 1:N){
    filename<-paste(DipMex,"/data/fromWeb/tablas/59/",tabs59bis[n],".txt",sep="")
    tmp <- read.table(filename)
    tmp <- as.vector(tmp[,]) ## NO SÉ POR QUÉ TENGO QUE HACER ESTO PARA QUE EL OBJETO QUEDE COMO VECTOR
    tmp2 <- grep(tmp, pattern="nomtit")
    tmp3 <- sub(tmp[tmp2], pattern=".*VALUE=(.*)$", replacement="\\1")
...
n <- 4
for (n in 1:N){
    filename<-paste(DipMex,"/data/fromWeb/tablas/61/",tabs61[n],".txt",sep="")
    tmp <- read.table(filename)
    tmp <- as.vector(tmp[,]) ## NO SÉ POR QUÉ TENGO QUE HACER ESTO PARA QUE EL OBJETO QUEDE COMO VECTOR
    tmp2 <- grep(tmp, pattern="nomtit")
    tmp3 <- sub(tmp[tmp2], pattern=".*VALUE=(.*)<p>.*$", replacement="\\1")
...
#
dat$yr <- as.numeric(dat$yr) ## OJO: LA VERSION QUE MANDÉ 8MAY11 A CANTU Y DESPOSATO LO DEJABA COMO CHARACTER, LO QUE CAMBIA EL SORT ORDER Y PLT EL FOLIO
...
for (n in 1:length(tmp)){
    ...
    tmp[n] <- gsub(tmp[n], pattern="\\Q]\\E", replacement="")
    tmp[n] <- gsub(tmp[n], pattern="\\Q$\\E", replacement="")
    tmp[n] <- gsub(tmp[n], pattern="\\Qº\\E", replacement="")
    tmp[n] <- gsub(tmp[n], pattern="\\Q°\\E", replacement="")
...
filenames<-paste(DipMex, "/data/fromWeb/votes/",
...
for (n in work){
    ...
    filename<-paste(DipMex, "/data/fromWeb/votes/",
...
workdir <- c("/home/eric/Dropbox/data/rollcall/dipMex/data/fromWeb/votes/60/")
#workdir <- c("d:/01/dropbox/data/rollcall/DipMex/data/fromWeb/votes/")
#workdir <- c("C:/Documents and Settings/emm/Mis documentos/My Dropbox/data/rollcall/DipMex/data/fromWeb/votes/")
#workdir <- c("C:/Documents and Settings/emagarm/Mis documentos/My Dropbox/data/rollcall/DipMex/data/fromWeb/votes/")
setwd(workdir)
...
#
filenames <- paste("sil60vot", vot60, ".txt", sep = "")
...
for (n in work){
    ...
    fch <- sub(tmp[1], pattern=".*DE LA SESIÓN DEL (.*)</u> <BR> (.*)</B>.*", replacement="\\1")
    tit <- sub(tmp[1], pattern=".*DE LA SESIÓN DEL (.*)</u> <BR> (.*)</B>.*", replacement="\\2")
...
save(filenames, file = "/home/eric/Dropbox/data/rollcall/dipMex/data/infosilFilenames60.RData")
...
rm(list = ls())
DipMex <-  c("/home/eric/Dropbox/data/rollcall/dipMex")
workdir <- c("/home/eric/Dropbox/data/rollcall/dipMex/data/fromWeb/votes/61/")
#workdir <- c("C:/Documents and Settings/emm/Mis documentos/My Dropbox/data/rollcall/DipMex/data/fromWeb/votes/")
#workdir <- c("C:/Documents and Settings/emagarm/Mis documentos/My Dropbox/data/rollcall/DipMex/data/fromWeb/votes/")
setwd(workdir)
...
filenames <- paste(workdir, "sil61vot", vot61, ".txt", sep = "")
dontdo <- file.exists( filenames )
tmp <- 1:length(dontdo)
work <- tmp[dontdo=="FALSE"]
...
save(filenames, file=paste(DipMex,"/data/infosilFilenames61.RData",sep=""))
...
workdir <- paste( DipMex, "data/fromWeb/votes/62/", sep = "/" )
setwd(workdir)

# download files not in infopal but in gaceta
# This loop identifies and downloads gaceta's summary tables. I will need to figure how roll call data is called from within--this appears to be done with lola[51] for pty 1 absences etc... python should do it well
fls62gaceta <- c("tabla1or1-3", "tabla1or1-10", "tabla1or1-25", "tabla1or1-27", "tabla1or1-31", "tabla1or1-33", "tabla1or1-64", "tabla1or1-65", "tabla1or1-67", "tabla1or1-68", "tabla1or1-69", "tabla1or1-70", "tabla1or2-4", "tabla1or2-5", "tabla1or2-37", "tabla1or2-103", "tabla1or2-118", "tabla2or1-39", "tabla2or1-40", "tabla2or1-43", "tabla2or1-44", "tabla2or1-50", "tabla2or1-59", "tabla2or1-60", "tabla2or1-61", "tabla2or1-63", "tabla2or1-66", "tabla2or1-69", "tabla2or1-74", "tabla2or1-75", "tabla2or1-101", "tabla2or1-102", "tabla2or1-103", "tabla2or1-104", "tabla2or1-127", "tabla2or1-128", "tabla2or1-132", "tabla2or2-49", "tabla2or2-51", "tabla2or2-510", "tabla2or2-702", "tabla2or2-703", "tabla2or2-818", "tabla2or2-819")
#
tmp <- c(3, 10, 29, 32, 36, 38, 75, 76, 78:81, 90, 91, 127, 199, 215, 297, 298, 307, 308, 314, 323:325, 327, 330, 333, 339, 340, 366:369, 394, 395, 399, 455, 457, 513, 528, 529, 548, 549) # infopal vote numbers that are filled with gaceta info
savenames <- paste("gac62vot", tmp, ".txt", sep = "")
dontdo <- file.exists( paste(workdir, savenames, sep = "") )
tmp <- 1:length(fls62gaceta)
work <- tmp[dontdo=="FALSE"]; rm(dontdo, tmp)
#
path <- "http://gaceta.diputados.gob.mx/Gaceta/Votaciones/62/"
n <- 1
for (n in work){
    direc <- paste(path, fls62gaceta[n], ".php3", sep="")
    url <- url(direc, open="r")
    tmp <- c("fch = ", "leg = 62", fls62gaceta[n], readLines(url, encoding = "latin1")) #"encoding = "ISO-8859-1"))
    close(url)
    write.table(tmp, filename, sep="\t")
    write(tmp, paste(DipMex, "/data/fromWeb/votes/62/", savenames[n], sep=""))
    print( paste("iteration =", n, "of", max(work)) )
}

# download infopal files
vot62 <- c(1, 2, 4:9, 11:17, 19:21, 25:28, 31, 33:35, 37, 39:41, 43:46, 49:54, 56:59, 61:69, 71:74, 77, 82:86) ## periodo 1.1; LIKE 60th, INFOPAL SKIPS SOME NUMBERS
#
# THIS ADDS VOTES
newvots <- c(87:89, 92:94, 96:116, 118, 120:126, 128:130, 133:155, 159:194, 196:198, 200:204, 206:214, 216:243, 245:250, 252)  ## periodo 1.2
tmp.nv1.2 <- length(newvots) # was useful to determine how many newvotes (files) added, looks obsolete in this version of code
newvots <- c(newvots, 253:256)  # periodo 1.3
tmp.nv1.3 <- length(newvots) - tmp.nv1.2
newvots <- c(newvots, 257:295, 299, 306, 309:313, 315:322, 326, 328:329, 331:332, 334:337, 341:365, 370:376, 378:391, 393, 396:398, 400:404)  # periodo 2.1
tmp.nv2.1 <- length(newvots) - tmp.nv1.2 - tmp.nv1.3
newvots <- c(newvots, 405:430, 432:446, 448:454, 456, 458:471, 473:510)  # periodo 2.2
tmp.nv2.2 <- length(newvots) - tmp.nv1.2 - tmp.nv1.3 - tmp.nv2.1
newvots <- c(newvots, 512, 514:520)  # periodo 2.3
tmp.nv2.3 <- length(newvots) - tmp.nv1.2 - tmp.nv1.3 - tmp.nv2.1 - tmp.nv2.2
newvots <- c(newvots, 522:526)  # periodo 2.4
tmp.nv2.4 <- length(newvots) - tmp.nv1.2 - tmp.nv1.3 - tmp.nv2.1 - tmp.nv2.2 - tmp.nv2.3
newvots <- c(newvots, 527, 530)  # periodo 2.5
tmp.nv2.5 <- length(newvots) - tmp.nv1.2 - tmp.nv1.3 - tmp.nv2.1 - tmp.nv2.2 - tmp.nv2.3 - tmp.nv2.4
newvots <- c(newvots, 531:547, 550:554) # periodo 2.6
tmp.nv2.6 <- length(newvots) - tmp.nv1.2 - tmp.nv1.3 - tmp.nv2.1 - tmp.nv2.2 - tmp.nv2.3 - tmp.nv2.4 - tmp.nv2.5
newvots <- c(newvots, 556:580) # periodo 3.1 (hasta 7oct2014)
tmp.nv3.1 <- length(newvots) - tmp.nv1.2 - tmp.nv1.3 - tmp.nv2.1 - tmp.nv2.2 - tmp.nv2.3 - tmp.nv2.4 - tmp.nv2.5 - tmp.nv2.6
#
vot62 <- c(vot62, newvots)
#
# TO ADD VOTES, REPEAT/ADAPT METHOD IN LINES ABOVE (AND newfilenames BELOW)
#
pp <- c(3,  ## pan
        1,  ## pri
        2,  ## prd
...
webmc    <- paste(path, pp[6], "&votaciont=", vot62, sep="")
webpanal <- paste(path, pp[7], "&votaciont=", vot62, sep="")
webindep <- paste(path, pp[8], "&votaciont=", vot62, sep="")
#
rm(tmp.nv1.2, tmp.nv1.3, tmp.nv2.1, tmp.nv2.2, tmp.nv2.3, tmp.nv2.4, tmp.nv2.5, tmp.nv2.6, tmp.nv3.1) 
#
filenames <- paste(workdir, "sil62vot", vot62, ".txt", sep = "")
newfilenames <- paste(workdir, "sil62vot", newvots, ".txt", sep = "")
dontdo <- file.exists( filenames )
tmp <- 1:length(dontdo)
work <- tmp[dontdo=="FALSE"]
...
#    tit <- sub(tmp[1], pattern=".*DE LA SESI\xd3N DEL (.*)</u> <BR> (.*)</B>.*", replacement="\\2")
    tmp2 <- grep(tmp, pattern="Sentido del voto")
    tmp <- tmp[(tmp2+1):length(tmp)] ## REMOVES JUNK TIL START OF NAMES LIST
    vote <- c( paste("fch =", fch), paste("leg =", leg), sub (filenames[n], pattern=".*(yr.*).txt", replacement="\\1"), tit, "emmPanVoteStart")
    vote <- append ( vote, tmp )
    #
    ## pri
    ...
    #
    ## mc
    url <- url( webmc[n], open="rt", encoding = "ISO-8859-1" )
    tmp<- readLines(url, warn=FALSE)
    close(url)
    tmp2 <- grep(tmp, pattern="Sentido del voto")
    tmp <- c( "emmMcVoteStart", tmp[(tmp2+1):length(tmp)] ) ## REMOVES JUNK TIL START OF NAMES LIST
    vote <- append ( vote, tmp )
    #
    ## panal
...
##
## SAVE INFOSIL FILENAMES INFO FOR USE IN NEW RC SYSTEMATIZATION
#filenames <- newfilenames # use if older files have been processed to systematize votes
save(filenames, file=paste(DipMex,"/data/infosilFilenames62.RData",sep=""))
## END OF INFOSIL ROLL CALL DOWNLOAD

load(file = paste(DipMex,"/data/infosilFilenames62.RData",sep="") )
ls()
head(filenames)
...
# SPOTS NUMBERS OUT OF SEQUENCE FOR EACH PERIODO # emm 11aug2014 looks like deprecated, I list numbers above now
tmp <- sub(dat$filename, pattern="tabla", replacement="")
