...
load(paste(workdir, "/ip61_2Dj_5+5k_gaceta.RData", sep=""))
I_gaceta <- I; votdat_gaceta <- votdat; dipdat_gaceta <- dipdat; ip2Dj_gaceta <- results; rm(results)
load(paste(workdir, "/ip61_2Dj_5+5k.RData", sep=""))
ip2Dj <- results
...
## PLOT CHAINS TO CHECK CONVERGENCE ### BETTER TO USE TRACEPLOT ON BUGS OBJECT
...
post.draw <- ip2Dj$sims.matrix 
post.draw_gaceta <- ip2Dj_gaceta$sims.matrix 
...
## STANDARDIZE X SCALE
post.x <- (post.x - mean(as.vector(post.x)))*10/sd(as.vector(post.x))
...
## STANDARDIZE Y SCALE
post.y <- (post.y - mean(as.vector(post.y)))*10/sd(as.vector(post.y))
...
dimnames(post.a) <- NULL; dimnames(post.b) <- NULL; dimnames(post.d) <- NULL;
...
#par(mai=c(.6, .6, .6, .6)) ## SETS B L U R MARGIN SIZES
plot(c(-3,2), c(-2.5,2.5), type="n", 
#plot(c(min(jotas[,2]),max(jotas[,2])), c(min(jotas[,5]),max(jotas[,5])), type="n", 
#plot(c(min(jotas[,2],jotas_g[,2]),max(jotas[,2], jotas_g[,2])), c(min(jotas[,2],jotas_g[,2]),max(jotas[,2],jotas_g[,2])), type="n", 
           xlab="", 
...
#           main="61st Leg. SMDs 2009-11")
           main="61st Leg. 2009-12")
#           main="Dimension 1")
#           main="")
#abline(0,1)
points(jotas[,2],jotas[,5], pch=19, cex=.75, col=dipdat$color)
#select <- dipdat$dsm==1 | dipdat$dsm==0
#points(jotas[select,2],jotas[select,5], pch=19, cex=.75, col=dipdat$color[select])
#points(jotas[,2],jotas[,5], col=dipdat$dcoord); ## pone coordinadores
#legend(-3,2.5, legend=part.list, cex=.75, pch=20, pt.cex=1.25, col=color.list, bg="white")
#points(jotas[,2],jotas_g[,2], pch=19, cex=.75, col=dipdat$color)

## CON CUTLINES
amed <- rep(NA,times=I)
bmed <- rep(NA,times=I)
for (i in 1:I){
    amed[i] <- quantile (post.a[,i], 0.50, names=F)
    bmed[i] <- quantile (post.b[,i], 0.50, names=F)  }
#
plot(c(-3,2), c(-2.5,2.5), type="n", 
           xlab=c(""), 
           ylab=c(""), 
...
for (i in 1:32){
    ...
    if (shrup$ndip[i,1]>1) {shrup$x[i,1]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$x[i,1]  <- NA};
    tmp <- dismed$xplus[,(dipdat$dsmd==1 & dipdat$part=="pri" & dipdat$edon==i)];
    if (shrup$ndip[i,2]>1) {shrup$x[i,2]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$x[i,2]  <- NA};
    tmp <- dismed$xplus[,(dipdat$dsmd==1 & dipdat$part=="prd" & dipdat$edon==i)];
    if (shrup$ndip[i,3]>1) {shrup$x[i,3]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$x[i,3]  <- NA};
    tmp <- dismed$xplus[,(dipdat$dsmd==1 & dipdat$part=="pt" & dipdat$edon==i)];
    if (shrup$ndip[i,4]>1) {shrup$x[i,4]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$x[i,4]  <- NA};
    tmp <- dismed$xplus[,(dipdat$dsmd==1 & dipdat$part=="pvem" & dipdat$edon==i)];
    if (shrup$ndip[i,5]>1) {shrup$x[i,5]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$x[i,5]  <- NA};
    tmp <- dismed$xplus[,(dipdat$dsmd==1 & dipdat$part=="conve" & dipdat$edon==i)];
    if (shrup$ndip[i,6]>1) {shrup$x[i,6]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$x[i,6]  <- NA};
    tmp <- dismed$xplus[,(dipdat$dsmd==1 & dipdat$part=="panal" & dipdat$edon==i)];
    if (shrup$ndip[i,7]>1) {shrup$x[i,7]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$x[i,7]  <- NA};
    tmp <- dismed$yplus[,(dipdat$dsmd==1 & dipdat$part=="pan" & dipdat$edon==i)];
    if (shrup$ndip[i,1]>1) {shrup$y[i,1]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$y[i,1]  <- NA};
    tmp <- dismed$yplus[,(dipdat$dsmd==1 & dipdat$part=="pri" & dipdat$edon==i)];
    if (shrup$ndip[i,2]>1) {shrup$y[i,2]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$y[i,2]  <- NA};
    tmp <- dismed$yplus[,(dipdat$dsmd==1 & dipdat$part=="prd" & dipdat$edon==i)];
    if (shrup$ndip[i,3]>1) {shrup$y[i,3]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$y[i,3]  <- NA};
    tmp <- dismed$yplus[,(dipdat$dsmd==1 & dipdat$part=="pt" & dipdat$edon==i)];
    if (shrup$ndip[i,4]>1) {shrup$y[i,4]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$y[i,4]  <- NA};
    tmp <- dismed$yplus[,(dipdat$dsmd==1 & dipdat$part=="pvem" & dipdat$edon==i)];
    if (shrup$ndip[i,5]>1) {shrup$y[i,5]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$y[i,5]  <- NA};
    tmp <- dismed$yplus[,(dipdat$dsmd==1 & dipdat$part=="conve" & dipdat$edon==i)];
    if (shrup$ndip[i,6]>1) {shrup$y[i,6]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$y[i,6]  <- NA};
    tmp <- dismed$yplus[,(dipdat$dsmd==1 & dipdat$part=="panal" & dipdat$edon==i)];
    if (shrup$ndip[i,7]>1) {shrup$y[i,7]  <- round(sum(tmp)/length(tmp), 2)} else {shrup$y[i,7]  <- NA};
    }
#
...
cplt <- function(X)
    ...
    plot(tmp+mean(as.vector(post.x)), tmp+mean(as.vector(post.y)), type="n",
               xlab="",
               ylab="",
               xaxt="n",
               yaxt="n",
...
legend(-3.25,2, legend=part.list[1:6], cex=.75, pch=3, pt.cex=1.25, col=color.list[1:6], bg="white")
##points(jotas[,5],jotas.gac[,5], pch=19, cex=.75, col=dipdat$color)


## EXTRACT FIRST-DIMENSION CUTPOINTS
post.cutpoint <- -post.b/post.a
point.post.d <- apply(post.d, 2, median)
point.post.x <- apply(post.x, 2, median)
point.post.cutpoint <- apply(post.cutpoint, 2, median)
## POSTERIOR FIRST-DIMENSION PREDICTIVE ACCURACY
v <- rc  
#v <- apply(v, 2, recode, recodes="-1=0; 0=NA; 1=1") ## RECODES VOTES TO 0,1
v <- apply(v, 2, recode, recodes="-1=0; 0=0; 1=1") ## RECODES VOTES TO 0,1 WITH NA AS 0
tmp <- rc; tmp[,] <- NA
post.mu <- tmp; post.v.star <- tmp; post.p <- tmp; post.v.hat <- tmp; rm(tmp)
##
for (i in 1:I){
  for (j in 1:J){
    post.mu[i,j] <- point.post.d[i]*(point.post.x[j] - point.post.cutpoint[i]);                 ## utility differential
...
##
## VOTES WHERE CUTPOINT IS BETWEEN PAN AND PRI IN FIRST-DIM
selectI <- point.post.cutpoint<point.medians$x[1] & point.post.cutpoint>point.medians$x[2]
#selectJ <- dipdat$dsmd==1 & (dipdat$edon==1 | dipdat$edon==6 | dipdat$edon==7 | dipdat$edon==9 | dipdat$edon==14 | dipdat$edon==18 | dipdat$edon==19 | dipdat$edon==22 | dipdat$edon==24 | dipdat$edon==29)
#
rice <- function()
    {
    tmp <- v[selectI,selectJ];
    tmp2 <- post.v.hat[selectI,selectJ];
    tmp3 <- rep(NA,nrow(tmp)); tmp4 <- tmp3; tmp5 <- tmp3; tmp6 <- tmp3; tmp7 <- tmp3; tmp8 <- tmp3;
    for (i in 1:nrow(tmp)){
        ayes <- length(tmp[i,is.na(tmp[i,])==FALSE & tmp[i,]==1])
        nays <- length(tmp[i,is.na(tmp[i,])==FALSE & tmp[i,]==0])
        tmp3[i] <- abs(ayes-nays) / (ayes+nays);
        tmp5[i] <- length(tmp[i,is.na(tmp[i,])==FALSE & tmp[i,]==1 & party[selectJ]==1])
        tmp6[i] <- length(tmp[i,is.na(tmp[i,])==FALSE & tmp[i,]==1 & party[selectJ]==2])
        tmp7[i] <- length(tmp[i,is.na(tmp[i,])==FALSE & tmp[i,]==0 & party[selectJ]==1])
        tmp8[i] <- length(tmp[i,is.na(tmp[i,])==FALSE & tmp[i,]==0 & party[selectJ]==2])
        ayes <- length(tmp2[i,is.na(tmp2[i,])==FALSE & tmp2[i,]==1])
        nays <- length(tmp2[i,is.na(tmp2[i,])==FALSE & tmp2[i,]==0])
        tmp4[i] <- abs(ayes-nays) / (ayes+nays);
        }
    print(data.frame(actual=round(tmp3,2), 
                     expected=round(tmp4,2), 
                     dlarger=ifelse(tmp3>tmp4, 1, 0), 
                     dsignif=ifelse(tmp3>(tmp4+2*sd(abs(tmp3[is.na(tmp3)==FALSE]-tmp4[is.na(tmp4)==FALSE]))), 1, 0),
                     ayespan=tmp5, ayespri=tmp6, nayspan=tmp7, nayspri=tmp8))
    }
s <- 1;
selectJ <- dipdat$dsmd==1 & (dipdat$edon==s);
tmp01 <- rice(); 
s <- 6;
selectJ <- dipdat$dsmd==1 & (dipdat$edon==s);
tmp06 <- rice(); 
...
tmp29 <- rice(); 
##
testelem <- list(ags=tmp01, col=tmp06, cps=tmp07, df=tmp09, jal=tmp14, nay=tmp18, nl=tmp19, que=tmp22, san=tmp24, tla=tmp29)
test <- data.frame(shr=rep(NA,10), sig=rep(NA,10)); ## shr is share of votes with actual>expected; sig act>exp+2sd
rownames(test) <- names(testelem)
for (s in 1:10){
    test[s,1] <- round(sum(testelem[[s]][,3])/length(testelem[[s]][,3]),2)
    test[s,2] <- round(sum(testelem[[s]][,4])/length(testelem[[s]][,4]),2)
    }

## DIMENSION-1 PLOT CON CUTLINES
#
## SELECT A STATE OR SMD OR BOTH TO HIGHLIGHT THEIR IPs IN PLOT
selectJ <- dipdat$edon==14 & dipdat$dsmd==1
## VOTES WHERE CUTPOINT IS BETWEEN PAN AND PRI IN FIRST-DIM
selectI <- point.post.cutpoint<point.medians$x[1] & point.post.cutpoint>point.medians$x[2]
jitter <- runif(J,-1,1)/5
plot(c(-3,2), c(-2,2), type="n",
           xlab=c("First-dimension scores"),
           ylab=c(""),
           xaxt="n",
           yaxt="n",
           bty="n",
           main="61st Leg. 2009-11")
    for (i in 1:I){ ## OJO: a en mi modelo es slope, en R es constant
        abline(a=bmed[i], b=amed[i], col="lightgrey")
        }
    for (i in c(1:I)[selectI]){ ## OJO: a en mi modelo es slope, en R es constant
        abline(a=bmed[i], b=amed[i], col="darkgrey")
        }
abline(h=0)
#rect(-5,.5,5,5, col="white", border="white")
#rect(-5,-5,5,-.5, col="white", border="white")
abline(v=point.medians[1:3,1], col=color.list[1:3])
text(point.medians[1,1]-.1, 1.25, label="PAN median", srt=90)
text(point.medians[2,1]-.1, 1.25, label="PRI median", srt=90)
text(point.medians[3,1]-.1, 1.25, label="PRD median", srt=90)
    for (j in 1:J){
        points(jotas[j,2],jitter[j],pch=20,col=dipdat$color[j], cex=1)
        }
    for (j in c(1:J)[selectJ]){ ## REPITE PUNTOS SELECCIONADOS CON HALO
        points(jotas[j,2],jitter[j],pch=20,col="#FFEBCD", cex=1.5)
        points(jotas[j,2],jitter[j],pch=20,col=dipdat$color[j], cex=1)
        }
axis(side=1, pos=0, at=-3:2)

## 2-DIM POSTERIOR PREDICTIVE ACCURACY
point.post.a <- apply(post.a, 2, median)
point.post.b <- apply(post.b, 2, median)
point.post.y <- apply(post.y, 2, median)
point.post.alpha <- apply(post.alpha, 2, median)
point.post.beta <- apply(post.beta, 2, median)
point.post.delta <- apply(post.delta, 2, median)
v <- rc  
v <- apply(v, 2, recode, recodes="-1=0; 0=NA; 1=1") ## RECODES VOTES TO 0,1
tmp <- rc; tmp[,] <- NA
post.mu <- tmp; post.v.star <- tmp; post.p <- tmp; post.v.hat <- tmp; rm(tmp)
...
for (i in 1:I){
  for (j in 1:J){
#    post.mu[i,j] <- point.post.d[i]*point.post.a[i]*point.post.x[j] 
#                    + point.post.d[i]*point.post.b[i] 
#                    - point.post.d[i]*point.post.y[j];                                   ## utility differential
    post.mu[i,j] <- point.post.beta[i]*point.post.x[j] 
                    - point.post.alpha[i] 
                    + point.post.delta[i]*point.post.y[j];                                   ## utility differential
    post.v.hat[i,j] <- ifelse( post.mu[i,j]>0, 1, 0 );                        ## voting rule
...
## PERCENT VARIANCE IN FIRST DIM UN-EXPLAINED BY PARTIES
## OJO: MEMO PARECE HACERLO CON VARIANZA, PERO ALGO ME SALE MAL
dpan <- rep (0, J); dpan <- ifelse (party==1, 1, 0); #dpan <- rep(dpan, 300)
dpri <- rep (0, J); dpri <- ifelse (party==2, 1, 0); #dpri <- rep(dpri, 300)
dprd <- rep (0, J); dprd <- ifelse (party==3, 1, 0); #dprd <- rep(dprd, 300)
dpt <- rep (0, J); dpt <- ifelse (party==4, 1, 0); #dpt <- rep(dpt, 300)
dpvem <- rep (0, J); dpvem <- ifelse (party==5, 1, 0); #dpvem <- rep(dpvem, 300)
dconve <- rep (0, J); dconve <- ifelse (party==6, 1, 0); #dconve <- rep(dconve, 300)
dpanal <- rep (0, J); dpanal <- ifelse (party==7, 1, 0); #dpanal <- rep(dpanal, 300)
#
tmp <- lm (jotas[,2] ~ dpan + dprd + dpt + dpvem + dconve + dpanal)
mean (sqrt(residuals(tmp)^2)) / sd(jotas[2,]) ## PCT SD NOT EXPLAINED BY PARTY
#
select <- dipdat$dsmd==1
tmp <- lm (jotas[select,2] ~ dpan[select] + dprd[select] + dpt[select] + dpvem[select] + dconve[select])
mean (sqrt(residuals(tmp)^2)) / sd(jotas[select,2]) ## PCT SD NOT EXPLAINED BY PARTY
#
tmp <- lm (as.vector(t(post.x)) ~ rep(dpan, 300) + rep(dprd, 300) + rep(dpt, 300) + rep(dpvem, 300) + rep(dconve, 300) + rep(dpanal, 300))
mean (sqrt(residuals(tmp)^2)) / sd(as.vector(post.x)) ## PCT SD NOT EXPLAINED BY PARTY
#
select <- dipdat$dsmd==1
tmp <- lm (as.vector(t(post.x[,select])) ~ rep(dpan[select], 300) + rep(dprd[select], 300) + rep(dpt[select], 300) + rep(dpvem[select], 300) + rep(dconve[select], 300) + rep(dpanal[select], 300))
mean (sqrt(residuals(tmp)^2)) / sd(as.vector(post.x)) ## PCT SD NOT EXPLAINED BY PARTY
